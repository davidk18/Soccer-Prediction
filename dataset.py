...
class Dataset:
    def __init__(self, file_path):
        ...
        self.result_info = [[] for i in range(3)]
        ...
        for result in self.raw_results:
            ...
            self.result_info[0].append(result['HomeTeam'])
            self.result_info[1].append(result['AwayTeam'])
            self.result_info[2].append(result['Date'])

    # Filter results to only contain matches played in by a certain team, before a certain date
    ...
    def get_statistics(self, team, date, matches=10):
        ...
        # Maps a result to a set of performance measures scaled between -1 and 1
        def map_fn(result):
